...
from ..forms import CUSTOMFIELD_DATE_FORMAT
from ..query import get_query_class, query_to_base64, query_from_base64
from ..user import HelpdeskUser

from ..decorators import helpdesk_staff_member_required, helpdesk_superuser_required, is_helpdesk_staff
from ..forms import TicketForm, UserSettingsForm, EmailIgnoreForm, EditTicketForm, TicketCCForm, TicketCCEmailForm, TicketCCUserForm, EditFollowUpForm, TicketDependencyForm, MultipleTicketSelectForm
from ..decorators import superuser_required
from ..lib import safe_template_context, process_attachments, queue_template_context
from ..models import Ticket, Queue, FollowUp, TicketChange, PreSetReply, FollowUpAttachment, SavedSearch, IgnoreEmail, TicketCC, TicketDependency, UserSettings, KBItem, CustomField, TicketCustomFieldValue
from ..settings import settings as helpdesk_settings
from ..views import abstract_views as abstract_views
from ..views.permissions import MustBeStaffMixin
...
def dashboard(request):
    ...
    if request.user.is_authenticated and hasattr(request.user, 'usersettings_SB'):
        tickets_per_page = request.user.usersettings_SB.tickets_per_page
    else:
        tickets_per_page = 25
...
def followup_edit(request, ticket_id, followup_id):
    ...
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            # delete old followup
            followup.delete()
        return HttpResponseRedirect(reverse('view', args=[ticket.id]))
...
def update_ticket(request, ticket_id, public=False):
    ...
    if not (public or (
        ...
        if not ticket:
            return HttpResponseRedirect(reverse('login'), request.path)
...
def mass_update(request):
    ...
    if action.startswith('kbitem_'):
        ...
        return redirect(
            reverse('merge_tickets') + '?' + '&'.join(['tickets=%s' % ticket_id for ticket_id in tickets])
        )
...
def ticket_list(request):
    ...
    return render(request, 'helpdesk/ticket_list.html', dict(
        context,
        default_tickets_per_page=UserSettings.tickets_per_page,
...
def save_query(request):
    ...
    return HttpResponseRedirect(reverse('list'), query.id)
...
def email_ignore_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse('email_ignore'))
    else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user and ticket.ticketcc_set.filter(user=user).exists():
                ...
                ticketcc.save()
                return HttpResponseRedirect(reverse('ticket_cc', kwargs={'ticket_id': ticket.id}))
...
def ticket_cc_del(request, ticket_id, cc_id):
    ...
    if request.method == 'POST':
        cc.delete()
        return HttpResponseRedirect(reverse('ticket_cc', kwargs={'ticket_id': cc.ticket.id}))
...
def ticket_dependency_del(request, ticket_id, dependency_id):
    ...
    if request.method == 'POST':
        dependency.delete()
        return HttpResponseRedirect(reverse('view', args=[ticket_id]))
