...
from ..decorators import protect_view, is_helpdesk_staff
from ..views import staff
from ..views import abstract_views
from ..lib import text_is_spam
from ..models import Ticket, Queue, UserSettings
from ..user import huser_from_request
...
class CreateTicketIframeView(BaseCreateTicketView):
    ...
    def form_valid(self, form):
        if super().form_valid(form).status_code == 302:
            return HttpResponseRedirect(reverse('success_iframe'))
...
def view_ticket(request):
    ...
    if is_helpdesk_staff(request.user):
        redirect_url = reverse('view', args=[ticket_id])
        if 'close' in request.GET:
            redirect_url += '?close'
        return HttpResponseRedirect(redirect_url)

    if 'close' in request.GET and ticket.status == Ticket.RESOLVED_STATUS:
        from ..views.staff import update_ticket
